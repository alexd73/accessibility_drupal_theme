...
// Add Zen Tabs styles
if (theme_get_setting('accessibility_tabs')) {
  drupal_add_css( drupal_get_path('theme', 'accessibility') . '/css/tabs.css');
}
 ...
 *   Template variables.
 */
function accessibility_preprocess_html(&$vars) {
  if (!module_exists('conditional_styles')) {
    accessibility_add_conditional_styles();
  }
...
function accessibility_preprocess_node(&$vars) {
  // Add a striping class.
  $vars['classes_array'][] = 'node-' . $vars['zebra'];
}

function accessibility_preprocess_block(&$vars, $hook) {
  // Add a striping class.
...
function accessibility_breadcrumb($variables) {
  ...
  $show_breadcrumb = theme_get_setting('accessibility_breadcrumb');
  if ($show_breadcrumb == 'yes' || ($show_breadcrumb == 'admin' && arg(0) == 'admin')) {



    // Optionally get rid of the homepage link.
    $show_breadcrumb_home = theme_get_setting('accessibility_breadcrumb_home');
    if (!$show_breadcrumb_home) {
      array_shift($breadcrumb);
    }
    // Return the breadcrumb with separators.
    if (!empty($breadcrumb)) {
      $breadcrumb_separator = theme_get_setting('accessibility_breadcrumb_separator');
      $trailing_separator = $title = '';
      if (theme_get_setting('accessibility_breadcrumb_title')) {
        $item = menu_get_item();
      ...
      elseif (theme_get_setting('accessibility_breadcrumb_trailing')) {
        $trailing_separator = $breadcrumb_separator;
      }
 ...
 */
function accessibility_add_conditional_styles() {
...
function accessibility_menu_link(array $variables) {
 ...
 */
function accessibility_preprocess_menu_local_task(&$variables) {
...
function accessibility_menu_local_tasks(&$variables) {
  $output = '';
